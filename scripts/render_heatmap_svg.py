...
Mirrors AVIVASHISHTA29's renderer: a grid of rounded, colored boxes in the
classic 53-week x 7-day calendar, revealed once with a diagonal
line-after-line slide-down (CSS keyframes, plays on load then freezes), month
+ weekday labels, a Less->More legend, and a real stats footer.

Pure standard library. Run by .github/workflows/update-profile-art.yml after
...
def render(data):
    days = data["days"]
    if not days:
        raise SystemExit("data/contributions.json has no days — run fetch_contributions.py")
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w / 2}" y="{TITLEBAR_H / 2 + 4}" fill="{MUTED}" '
                 f'font-size="12" text-anchor="middle">{PROMPT}: ~/contributions --graph</text>')
    ...
    for wi, wname in [(1, "Mon"), (3, "Wed"), (5, "Fri")]:
        ...
        parts.append(f'<text x="{PAD}" y="{y:.1f}" fill="{MUTED}" '
                     f'font-size="9">{wname}</text>')
